...
export async function syncFeedsOnce() {
  ...
  for (const feed of feeds) {
    ...
    try {
      ...
      console.log('syncFeedsOnce: Fetch result for', feed.title, '- Status:', res.status, 'Text length:', res.text?.length);
      ...
      if (res.text) {
        ...
        console.log('syncFeedsOnce: Parsed', items.length, 'items for', feed.title);
        
        for (const item of items) {
          console.log('syncFeedsOnce: Adding article:', item.title, 'Image:', !!item.image, 'Content:', !!item.contentHtml);
