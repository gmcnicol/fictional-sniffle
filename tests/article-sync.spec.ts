...
test.describe('Article Sync End-to-End', () => {
  test('should add QC feed, sync articles, and display them with screenshots', async ({
    page,
  }) => {
    ...
    await page.screenshot({
      path: 'test-results/02-feed-added.png',
      fullPage: true,
    });
    ...
    const feedCount = await page
      .locator('ul li')
      .filter({ hasText: 'questionablecontent.net' })
      .count();
    ...
    await page.screenshot({
      path: 'test-results/03-after-sync.png',
      fullPage: true,
    });
    ...
    const hasArticleContent =
      pageContent?.includes('X Versus O') ||
      pageContent?.includes('Loading Zone') ||
      pageContent?.includes('Not-It');
    ...
    if (articleItems > 0) {
      ...
      console.log(
        'Articles section exists but no articles loaded yet - check screenshots and logs',
      );
    }
  });
});
