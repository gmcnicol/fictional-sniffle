...
test.describe('QC Feed Integration', () => {
  test('should add QC feed and display articles', async ({ page }) => {
    ...
    const feedElements = await page.locator('text=QC RSS, text=Questionable Content').count();
    console.log('Feed elements found:', feedElements);
    
    // Check if there's an error message (but don't wait forever)
    try {
      const errorMessage = await page.locator('div[style*="color: red"]').textContent({ timeout: 2000 });
      if (errorMessage) {
    ...
    // Check for articles
    const articleElements = await page.locator('ul li a[href*="/reader/"]').count();
  ...
  test('should show detailed sync information', async ({ page }) => {
    ...
    const articleCount = await page.evaluate(async () => {
      // @ts-ignore - Access IndexedDB in browser
