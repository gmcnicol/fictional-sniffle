...
test('complete comic viewer functionality - add feed, sync, view comic', async ({ page }) => {
  // Ignore network errors (CORS/ERR_FAILED are expected)
  page.on('console', msg => {
    if (msg.type() === 'error' && (msg.text().includes('CORS') || msg.text().includes('ERR_FAILED'))) {
  ...
  await page.screenshot({ path: 'test-results/comic-feed-list.png', fullPage: true });
  ...
  await page.screenshot({ path: 'test-results/comic-reader-final.png', fullPage: true });
